...
version_is_newer() {
  ...
  for i in 0 1 2; do
    ...
    ((10#$lv > 10#$cv)) && return 0
    ((10#$lv < 10#$cv)) && return 1
  done
  return 1
}
...
update_installer() {
  ...
  if ! version_is_newer "$current" "$latest"; then
    log "The installer is already up to date."
    return 0
  fi

  ask_yes_no "Update installer from v${current} to v${latest}?" N || {
  ...
  [[ "$expected" =~ ^[a-fA-F0-9]{64}$ ]] || die "No valid SHA256 entry found for ${artifact}."
  actual="$(sha256sum "$tmp_artifact" | awk '{print $1}')"
  [[ "${actual,,}" == "${expected,,}" ]] || die "SHA256 verification failed. The installer was not changed."

  bash -n "$tmp_artifact" || die "Downloaded installer failed bash syntax validation."
  if command_exists shellcheck; then
    shellcheck --severity=warning "$tmp_artifact" || die "Downloaded installer failed ShellCheck validation."
  fi

  mkdir -p "$BACKUP_DIR"
  chmod 700 "$BACKUP_DIR"
  backup_path="$BACKUP_DIR/installer-v${current}-$(date +%Y%m%d-%H%M%S).sh"
...
get_vpn_users() {
  [[ -r "$IPSEC_SECRETS" ]] || return 0

  awk '$1 ~ /^[A-Za-z0-9._@-]+$/ && length($1) <= 64 && $2 == ":" && $3 == "EAP" && substr($4, 1, 1) == "\"" {print $1}' "$IPSEC_SECRETS"
}
...
configured_online_vpn_user_count() {
  local username
  local count=0

  if [[ "${VPN_SESSION_STATUS_AVAILABLE:-no}" != "yes" ]]; then
    printf 'unknown\n'
    return 0
  fi
...
select_vpn_user() {
  ...
  mapfile -t vpn_users < <(get_vpn_users)
  if (( ${#vpn_users[@]} == 0 )); then
    warn "No VPN users are currently configured."
    return 1
  fi

  printf '\n%bSelect VPN user%b\n\n' "$BOLD" "$RESET"
  for ((index=0; index<${#vpn_users[@]}; index++)); do
    printf '  %d) %s\n' "$((index + 1))" "${vpn_users[$index]}"
  done
  printf '  0) Cancel\n\n'

  while true; do
    read -r -p "Choose [0-${#vpn_users[@]}]: " choice || true
    if [[ "$choice" == "0" ]]; then
      return 1
    fi
    if [[ "$choice" =~ ^[0-9]+$ ]]; then
      numeric_choice=$((10#$choice))
      if (( numeric_choice >= 1 && numeric_choice <= ${#vpn_users[@]} )); then
        SELECTED_VPN_USER="${vpn_users[$((numeric_choice - 1))]}"
        return 0
      fi
    fi
...
collect_new_vpn_password() {
  ...
  while true; do
    read -r -s -p "$prompt" password || true
    printf '\n'

    if [[ -z "$password" ]]; then
      NEW_VPN_PASSWORD=$(generate_password)
      VPN_PASSWORD_WAS_GENERATED="yes"
      return 0
    fi

    if [[ ${#password} -lt 12 ]]; then
      warn "Use a password with at least 12 characters."
      continue
    fi

    read -r -s -p 'Confirm password: ' password_repeat || true
    printf '\n'
    ...
    NEW_VPN_PASSWORD="$password"
    VPN_PASSWORD_WAS_GENERATED="no"
    return 0
  done
}

reload_vpn_secrets() {
  if command_exists ipsec && ipsec rereadsecrets >/dev/null 2>&1; then
    return 0
  fi

  warn "User configuration was updated, but StrongSwan could not reload secrets."
  warn "Run: ipsec rereadsecrets"
  return 0
}

list_vpn_users() {
  local -a vpn_users=()
  local index username status online_count

  require_managed_installation
  mapfile -t vpn_users < <(get_vpn_users)

  printf '\n%bConfigured VPN users%b\n' "$BOLD" "$RESET"
  printf '====================\n\n'
...
add_vpn_user() {
  ...
  if [[ ! -f "$IPSEC_SECRETS" ]]; then
    warn "VPN secrets file not found: ${IPSEC_SECRETS}"
    return 0
  fi
  ...
  while true; do
    ...
    if vpn_user_exists "$username"; then
      warn "VPN user '${username}' already exists."
      return 0
    fi
    break
  done

  collect_new_vpn_password 'Password (empty = generate): '
  escaped_password=$(escape_secret "$NEW_VPN_PASSWORD")

  if ! printf '%s : EAP "%s"\n' "$username" "$escaped_password" >> "$IPSEC_SECRETS"; then
    warn "Could not add VPN user '${username}'."
    return 0
  fi
  chmod 600 "$IPSEC_SECRETS"
  reload_vpn_secrets
  ...
  printf '\n'
  if [[ "$VPN_PASSWORD_WAS_GENERATED" == "yes" ]]; then
    log "VPN user added successfully."
    printf '\nUsername : %s\n' "$username"
    printf 'Password : %s\n' "$NEW_VPN_PASSWORD"
  else
    log "VPN user '${username}' added successfully."
  fi
}
...
change_vpn_user_password() {
  ...
  select_vpn_user || return 0

  refresh_online_vpn_users
  if [[ "$VPN_SESSION_STATUS_AVAILABLE" == "yes" ]] && vpn_user_is_online "$SELECTED_VPN_USER"; then
    info "User '${SELECTED_VPN_USER}' currently has an active VPN session."
    info "The new password will be required on the next authentication."
  fi

  collect_new_vpn_password 'New password (empty = generate): '
  escaped_password=$(escape_secret "$NEW_VPN_PASSWORD")
  if ! temp_file=$(mktemp "${IPSEC_SECRETS}.tmp.XXXXXX"); then
    warn "Could not create a temporary VPN secrets file."
    return 0
  fi
  ...
  if ! {
    while IFS= read -r line || [[ -n "$line" ]]; do
      ...
      separator=""
      secret_type=""
      IFS=      if [[ "$entry_username" == "$SELECTED_VPN_USER" && "$separator" == ":" && "$secret_type" == "EAP" ]]; then
    ...
    rm -f -- "$temp_file"
    warn "Could not update the VPN secrets file."
    return 0
  fi

  if ! chmod 600 "$temp_file" || ! mv -f -- "$temp_file" "$IPSEC_SECRETS"; then
    rm -f -- "$temp_file"
    warn "Could not replace the VPN secrets file."
    return 0
  fi

  reload_vpn_secrets
  log "Password changed successfully for '${SELECTED_VPN_USER}'."
  if [[ "$VPN_PASSWORD_WAS_GENERATED" == "yes" ]]; then
...
remove_vpn_user() {
  ...
  refresh_online_vpn_users
  if [[ "$VPN_SESSION_STATUS_AVAILABLE" == "yes" ]] && vpn_user_is_online "$SELECTED_VPN_USER"; then
    warn "VPN user '${SELECTED_VPN_USER}' is currently Online."
    warn "Removing the credentials will not necessarily terminate the existing IKEv2 session immediately."
  fi

  if (( user_count == 1 )); then
    warn "This is the last configured VPN user."
    warn "Removing it will leave the VPN without usable EAP credentials."
    ask_yes_no "Remove the last VPN user anyway?" N || {
      info "Removal canceled."
      return 0
    }
  else
    ask_yes_no "Remove VPN user '${SELECTED_VPN_USER}'?" N || {
      info "Removal canceled."
      return 0
    }
  fi
  ...
  reload_vpn_secrets
  log "VPN user '${SELECTED_VPN_USER}' removed successfully."
  info "Existing sessions for this user may remain active until disconnected or renegotiated."
}
